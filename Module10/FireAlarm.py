...
class Building:
    ...
    def run_elevator(self, elevator_id, floor):
        ...
        print(f"Run elevator: {elevator_id}, floor {floor}")

    def fire_alarm(self):
        for elevator_id, _ in enumerate(self.list_elevators):
            self.run_elevator(elevator_id, 1)
            print(f"Fire alarm: {elevator_id}, floor: {1}")
